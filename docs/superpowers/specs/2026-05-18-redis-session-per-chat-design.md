...
| `core/session.py` (new) | Small module owning the checkpointer lifecycle: `init_checkpointer()`, `close_checkpointer()`, `get_checkpointer()`, `ping()`, `validate_chat_id()`, `new_chat_id()`. Keeping it separate from `core/agent.py` so the agent factory stays single-purpose. TTL is handled internally by `AsyncRedisSaver` via its `ttl={"default_ttl": …, "refresh_on_read": True}` constructor argument — the library applies `EXPIRE` to every key on write and refreshes it on read, so no `touch_ttl` helper is needed. |
